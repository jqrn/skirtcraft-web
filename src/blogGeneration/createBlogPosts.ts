...
export const createBlogPosts: GatsbyCreatePages = async ({
  ...
  const filteredTumblrPostEdges =
    allTumblrPosts.data.allTumblrPost.edges.filter((edge: any) => {
  ...
  filteredTumblrPostEdges.forEach((edge: TumblrGraphqlEdge, index: number) => {
    ...
    createPage({
      ...
      context: {
        ...
        nextSlug:
          index > 0
            ? getBlogPostSlug(filteredTumblrPostEdges[index - 1].node)
            : undefined,
        previousSlug:
  ...
  const timelinePageCount = Math.ceil(
    filteredTumblrPostEdges.length / maxPostsPerTimelinePage
  );
  ...
  filteredTumblrPostEdges.forEach((edge: TumblrGraphqlEdge, index: number) => {
    ...
    if (
      currentTimelinePagePostIds.length >= maxPostsPerTimelinePage ||
      index >= filteredTumblrPostEdges.length - 1
    ) {
      createPage({
        path:
          currentTimelinePageNumber === 1
            ? 'blog'
            : `blog/page-${currentTimelinePageNumber}`,
