...
export default class HomePage extends React.PureComponent<null, State> {
  public state: State = {
    slideIndex: 0,
    isAuto: true,
  };
  ...
  public render(): JSX.Element {
    ...
    return (
      <Page
        title={NavTabDef.HOME}
        currentTab={NavTabDef.HOME}
        mainStyle={mainStyle}
      >
        <Container>
          <SlideContainer>
            ...
            <NextOrPreviousButton
              isprevious={true}
              onClick={() => this.shiftSlide(true, false)}
            >
              &#10094;
            </NextOrPreviousButton>
            <NextOrPreviousButton
              isprevious={false}
              onClick={() => this.shiftSlide(false, false)}
            >
              &#10095;
            </NextOrPreviousButton>
          </SlideContainer>
        ...
        <NavDotGroup>
          {SLIDES.map((_, index) => (
            <Dot
              ...
              onClick={() =>
                this.setState({ slideIndex: index, isAuto: false })
              }
            />
          ))}
        </NavDotGroup>
...
const NextOrPreviousButton = styled.div`
  ...
  border-radius: ${(props: { isprevious: boolean }) =>
    props.isprevious ? '0 3px 3px 0' : '3px 0 0 3px'};
  background-color: rgba(255, 255, 255, 0.65);
  right: ${(props: { isprevious: boolean }) =>
    props.isprevious ? undefined : 0};
  user-select: none;
  &: hover {
    border-color: black;
...
const Dot = styled.span`
  ...
  background-color: ${(props: { iscurrent: boolean }) =>
    props.iscurrent ? '#333' : '#aaa'};
  border-radius: 50%;
  display: inline-block;
  transition: background-color 0.6s ease;
  &: hover {
    background-color: ${(props: { iscurrent: boolean }) =>
      props.iscurrent ? '#333' : '#333'};
  }
`;
